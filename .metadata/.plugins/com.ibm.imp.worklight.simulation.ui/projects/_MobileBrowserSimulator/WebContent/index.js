			     		...
			     		dojo.forEach(response['platforms'], function(platform){ //fetch platforms from device list
			    		  ...
			    		  dojo.forEach(response['devices'], function(device){
			    		    	if ((platformID == device.platformID) || (platformID == null)) {
			    				    ...
			    				    var agentID_Worklight = device.agentID + "/Worklight/7.0";
			    				    var deviceItem = {
			    				        "type" : "device",
			    				        "id" : device.name,
			    				        "name" : device.name,
 ...
 /*	var wireQRCode = function() {
	...
	ready(function(){		
		...
		if (window.location.search) {
			...
			if (quary['iosVersion']) {
			     iosVersion = query['iosVersion'];
			}
			if (quary['iosVersionOn']) {
			     iosVersionOn = query['iosVersionOn'];
			}
